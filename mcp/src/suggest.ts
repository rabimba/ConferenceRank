/**
 * Abstract → venue suggester. Faithful port of the site's suggest.ts
 * (conf-rank/src/lib/suggest.ts), minus the neural-embedding blend (no
 * browser/WebAssembly here): category lexicon + TF-IDF cosine + ambition
 * weighting. Keep scoring constants in sync with the site.
 */
import {
  CATEGORY_LEXICON,
  METHODOLOGY_SIGNALS,
...
function extractKeywordsAndCategories(cleanStr: string): {
  ...
  for (const [category, lexicon] of Object.entries(CATEGORY_LEXICON)) {
    ...
    if (catScore > 0) {
      scores.push({ category, score: Math.round(catScore * 10) / 10, matchedTerms });
    }
  }
  ...
  for (const sig of METHODOLOGY_SIGNALS) {
    ...
    for (const kw of sig.keywords) {
      if (kw.includes(" ") ? cleanStr.includes(kw) : wordSet.has(kw)) matched.push(kw);
    }
    if (matched.length > 0) detectedMethodologies.push({ label: sig.label, keywords: matched });
  }
  return { categoryScores: scores, matchedKeywords: Array.from(foundKeywords), detectedMethodologies };
}
...
function determineTier(venue: SuggesterVenue): "stretch" | "target" | "safe" {
  ...
  if (
    venue.rank === "A" || venue.core_rank === "A" ||
    venue.sjr_quartile === "Q1" || venue.rank === "Q1"
  ) return "target";
...
function computeAmbitionAdjustment(
  rank: string, latestRate: number | null, ambition: AmbitionTier
): number {
  if (ambition === "all") return (RANK_BASE_WEIGHTS[rank] ?? 0.3) * 0.15;
  const isHighlySelective = rank === "A*" || (latestRate !== null && latestRate <= 20);
  const isModerate =
    rank === "A" || rank === "B" || rank === "Q1" || rank === "Q2" ||
    (latestRate !== null && latestRate > 20 && latestRate <= 32);
  const isAccessible =
    rank === "C" || rank === "Australasian B" || rank === "Australasian C" ||
    rank === "Q3" || rank === "Q4" || (latestRate !== null && latestRate > 32);
  if (ambition === "stretch") {
...
export function suggestVenues(
  ...
  if (isTooShort || !clean) {
    return {
      suggestions: [], detectedCategories: [], detectedMethodologies: [],
      detectedKeywords: [], wordCount, isTooShort: true, confidence: "low",
    };
  }
  ...
  const tfidf: TfidfIndex = buildTfidfIndex(
    venues.map((v) => ({
      id: v.id,
  ...
  for (const venue of venues) {
    if (options.venueType && options.venueType !== "all") {
      if ((venue.type ?? "conference") !== options.venueType) continue;
    }
    if (!includeUnranked && (venue.rank === "Unranked" || !venue.rank)) continue;

    let categoryAffinity = 0;
    const matchedCategories: string[] = [];
    for (const cat of venue.categories) {
      const w = topCategoryMap.get(cat);
      if (w !== undefined) {
        categoryAffinity = Math.max(categoryAffinity, w);
    ...
    for (const vt of venueTitleTokens) {
      if (tokenSet.has(vt) && !ENGLISH_STOPWORDS.has(vt)) titleMatches.push(vt);
    }
    const hasAcronymMention = venueAcronymLower.length >= 3 && tokenSet.has(venueAcronymLower);
    const titleOverlapScore = Math.min(titleMatches.length * 0.15 + (hasAcronymMention ? 0.35 : 0), 0.6);
    const simScore = Math.min(similarity(tfidf, venue.id, abstract) * 2.2, 0.6);

    if (categoryAffinity === 0 && titleOverlapScore === 0 && simScore < 0.15) continue;

    const ambitionAdjustment = computeAmbitionAdjustment(venue.rank, venue.latest_rate, ambition);

    // Site fallback formula (no neural embeddings in MCP): category 40% ·
    // title overlap 15% · doc TF-IDF 25% · ambition 20%.
    const rawScore =
      categoryAffinity * 0.4 + titleOverlapScore * 0.15 + simScore * 0.25 + ambitionAdjustment;
    ...
    if (matchedCategories.length > 0) {
      ...
      reasons.push(`${topCat} fit${termCount > 0 ? ` (${termCount} keyword${termCount > 1 ? "s" : ""})` : ""}`);
    }
    if (simScore >= 0.25) reasons.push("Strong text similarity to venue scope/topics");
    if (titleMatches.length > 0) reasons.push(`Title matches: ${titleMatches.slice(0, 3).join(", ")}`);
    if (venue.latest_rate !== null) reasons.push(`${venue.latest_rate}% acceptance rate`);

    scoredVenues.push({
      venue,
      totalScore: rawScore,
      matchPercentage,
      tier: determineTier(venue),
      matchedCategories,
